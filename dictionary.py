...
#Leitura de valores pre-definidos de p_value 
def readPValue():
...
def compareMotif(motif, matrixMotif):
	...
	return motifValue
def findAndCompareMotif(sequence, matrixMotif):
	#print(sequence)
	#print(len(matrixMotif))
	motif = ''
	toOut=''
	#Finding Motif
...
if __name__ == '__main__':
	...
	for k_Seq,v_Seq in listOfSeq.items():
		for k_Matx,v_Matx in matrix.items():
			#print(k_Matx)
			#print(k_Seq)
			motif = findAndCompareMotif(v_Seq, v_Matx)
			out.write('>'+k_Seq+ ' >'+k_Matx+' > '+motif[:-2])
			out.close()
			break
		break		
